...
/**/
/*
for (ival=500.0;ival<1800.0;ival+=50.0)
	{
ival1=1.5;
markw[0]=0.2;markt[0]=29;
meltpart1(ival+273.15,ival1*1e+4,0);
	}
*/
/**/
/* Open File mode.t3c */
...
if(m1==xnumx-1 && m2==0) {printf("E  %ld %d %e %e %e ",m1,yn,bx[0],bx[1],x); getchar();}
*/
			break;
			/**/
			/**/
...
*/
					...
					if(yn==5)
						{
						/* Transition 4->0 */
						ival=(y*(by[1]-by[0])*ysize)/2.0/pow(t[6]*(t[4]+(t[5]-t[4])*x)*3.15576e+7,0.5);
						ival=t[1]-(1.0-erf(ival))*(t[1]-t[0]);
						tk[m3]=t[2]+y*(t[3]-t[2]);
					...
					if(yn==6)
						{
						/* Transition 0->4 */
						ival=(y*(by[3]-by[2])*ysize)/2.0/pow(t[6]*(t[4]+(t[5]-t[4])*x)*3.15576e+7,0.5);
						ival=t[3]-(1.0-erf(ival))*(t[3]-t[2]);
						tk[m3]=t[0]+y*(t[1]-t[0]);
